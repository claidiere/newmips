...
const FILL = {
	none: (appName, capturingGroups) => '',
	applications: (appName, capturingGroups) => metadata.getApplications(),
	entities: (appName, capturingGroups) => metadata.getApplication(appName).entities.map(x => x.displayName),
	modules: (appName, capturingGroups) => metadata.getApplication(appName).modules,
	fields: (appName, capturingGroups) => {
		metadata[id_app].findEntity(entity).getFields();
	},
	fieldAttribute: (appName, capturingGroups) => ['required', 'readonly'],
	statuses: (appName, capturingGroups) => {
		// build status list
	},
	columnVisibility: (appName, capturingGroups) => ['hidden', 'visible'],
	fieldType: (appName, capturingGroups) =>  [
		'enum',
		'radio',
		"string",
		"text",
	...
	tab: (appName, capturingGroups) => {
		const opts = metadata[id_app].getName; // fetch options of current entity and get tabs traductions
		for (const opt of opts)
			;
	},
	agendas: (appName, capturingGroups) => '',
	layouts: (appName, capturingGroups) => {
		const baseLayouts = ['boxed', 'default', 'right-navigation', 'left-navigation'];
		const customLayouts = [];// fetch custom layouts
		return [...baseLayouts, ...customLayouts];
	},
	themes: (appName, capturingGroups) => '',
	icones: (appName, capturingGroups) => ['fa*'],
	widgets: (appName, capturingGroups) => ['last records', 'info', 'statistique'],
}
...
const training = {
	...
	"selectApplication": {
		autocomplete: [FILL.applications],
		instructions: [
			"select application (.*)",
	...
	"setFieldKnownAttribute": {
		autocomplete: [FILL.field, FILL.fieldAttribute],
		instructions: [
			"set field (.*) (.*)",
			"set the field (.*) (.*)",
			"mettre champ (.*) (.*)",
	...
	"setColumnVisibility": {
		autocomplete: [FILL.field, FILL.columnVisibility],
		instructions: [
			"set column (.*) (.*)",
	...
	"createNewDataFieldWithType": {
		autocomplete: [FILL.none, FILL.fieldType],
		instructions: [
	...
	"relationshipHasOne": {
		autocomplete: [FILL.entities, FILL.entities],
		instructions: [
			"entity (.*) has one (.*)",
			"entité (.*) possède un (.*)",
			"entité (.*) possède une (.*)",
	...
	"relationshipHasOneWithName": {
		autocomplete: [FILL.entities, FILL.entities, FILL.none],
		instructions: [
	...
	"createFieldRelatedTo": {
		autocomplete: [FILL.none, FILL.entities],
		instructions: [
	...
	"createFieldRelatedToUsing": {
		autocomplete: [FILL.none, FILL.entities, FILL.fields],
		instructions: [
	...
	"relationshipHasManyPresetUsing": {
		autocomplete: [FILL.entities, FILL.entities, FILL.fields, FILL.none],
		instructions: [
	...
	"createWidgetPiechart": {
		autocomplete: [FILL.entities, FILL.fields],
		instructions: [
	...
	"createWidgetLastRecordsWithLimit": {
		autocomplete: [FILL.none, FILL.fields],
		instructions: [
	...
	"createWidgetOnEntity": {
		autocomplete: [FILL.widgets, FILL.entities],
		instructions: [
...
exports.parse = (instruction) => {
	...
	for (const action in training) {
		for (let i = 0; i < training[action].instructions.length; i++) {
			const regStr = training[action].instructions[i];
...
exports.complete = function (inputInstruction, appName) {
	...
	for (const action in training) {
		...
		for (const trainingInstru of training[action].instructions) {
			...
			while (true) {
				// Bounds checks
				{
					// End of both array reached, go to next instruction
					if (!instruSplit[i] && !trainingSplit[i])
						continue actionInstructions;

					// Training longer, instruction not complete, add next training word / filler
					if (!instruSplit[i]) {
						answers = trainingSplit[i] == '(.*)' && training[action].autocomplete && training[action].autocomplete[capturingGroupCount]
								? [...answers, ...training[action].autocomplete[capturingGroupCount](appName, capturingGroupValues)]
								: [...answers, ...[trainingSplit[i]]];
						continue actionInstructions;
					}
					// Instruction longer, training not matching
					if (!trainingSplit[i])
						continue actionInstructions;
				}

				// Capturing group reformating instruSplit array
				{
					if (trainingSplit[i] == '(.*)') {
						capturingGroupCount++;
						// Training instruction ends on capturing group, add answers
						if (!trainingSplit[i+1]) {
							console.log('training end on capturing');
					...
					else if (trainingSplit[i] != instruSplit[i]) {
						if (trainingSplit[i].indexOf(instruSplit[i]) == 0)
							answers.push(trainingSplit[i]);
						continue actionInstructions;
					}
				}
				i++;
	...
	// 		const template = training[action].instructions[i].split(" ");
